...
export default function TradingQuestionnairePage() {
  ...
  const generateTasks = useCallback(async (workstreamId: string) => {
    ...
    try {
      const res = await fetch('/api/ops/compliance-tasks', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ missionId, moduleId: MODULE_ID, workstreamId, mode: 'from_analysis' }) });
      if (res.ok) { const data = await res.json(); setTasks((prev) => { const u = [...prev]; for (const t of data.tasks) { const i = u.findIndex((e) => e.id === t.id); if (i >= 0) u[i] = t; else u.push(t); } return u; }); }
    } catch (err) { console.error('Task generation failed:', err); } finally { setGeneratingTasks(null); }
  }, [missionId]);

  const acceptTask = useCallback(async (taskId: string) => {
    await fetch('/api/ops/compliance-tasks', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ taskId, status: 'not_started' }) });
