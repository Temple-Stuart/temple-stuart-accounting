...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string; optionId: string }> }) {
  try {
    ...
    const option = await prisma.trip_activity_expenses.findFirst({ where: { id: optionId, trip_id: id }, select: { id: true } });
    if (!option) return NextResponse.json({ error: 'Option not found' }, { status: 404 });

    const body = await request.json();

    if (body.action === 'vote_up') {
      await prisma.trip_activity_expenses.update({ where: { id: optionId }, data: { votes_up: { increment: 1 } } });
      return NextResponse.json({ success: true });
    }
    if (body.action === 'vote_down') {
      await prisma.trip_activity_expenses.update({ where: { id: optionId }, data: { votes_down: { increment: 1 } } });
      return NextResponse.json({ success: true });
    }
    if (body.action === 'select') {
      await prisma.trip_activity_expenses.update({ where: { id: optionId }, data: { is_selected: true } });
      return NextResponse.json({ success: true });
    }
    if (body.action === 'deselect') {
      await prisma.trip_activity_expenses.update({ where: { id: optionId }, data: { is_selected: false } });
      return NextResponse.json({ success: true });
    }

    const { title, vendor, url, price, per_person, notes } = body;
    await prisma.trip_activity_expenses.update({
      where: { id: optionId },
      data: {
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string; optionId: string }> }) {
  try {
    ...
    const option = await prisma.trip_activity_expenses.findFirst({ where: { id: optionId, trip_id: id }, select: { id: true } });
    if (!option) return NextResponse.json({ error: 'Option not found' }, { status: 404 });
    await prisma.trip_activity_expenses.delete({ where: { id: optionId } });
    return NextResponse.json({ success: true });
  } catch (error) {
