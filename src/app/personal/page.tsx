...
export default function PersonalPage() {
  ...
  const [form, setForm] = useState({ name: '', coa_code: 'P-6100', amount: '', cadence: 'monthly' });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const res = await fetch('/api/personal', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...form, amount: parseFloat(form.amount) }) });
    if (res.ok) { setShowForm(false); setForm({ name: '', coa_code: 'P-6100', amount: '', cadence: 'monthly' }); loadExpenses(); }
  };

  const handleAction = async (id: string, action: 'commit' | 'uncommit') => {
    if (!confirm(action === 'commit' ? 'Commit?' : 'Uncommit?')) return;
    setActionLoading(id);
    await fetch(`/api/personal/${id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action }) });
    loadExpenses(); setActionLoading(null);
  };

  const handleDelete = async (id: string) => {
    if (!confirm('Delete?')) return;
    await fetch(`/api/personal/${id}`, { method: 'DELETE' }); loadExpenses();
  };
