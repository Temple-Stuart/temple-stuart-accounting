...
const COA_OPTIONS = [
  { code: 'P-6100', name: 'Meals & Dining' },
  { code: 'P-6110', name: 'Coffee & Snacks' },
  { code: 'P-6120', name: 'Dating & Social' },
  { code: 'P-8170', name: 'Entertainment & Hobbies' },
  { code: 'P-8190', name: 'Subscriptions (Netflix, etc)' },
  { code: 'P-8200', name: 'Cell Phone' },
];
...
export default function PersonalPage() {
  ...
  const [form, setForm] = useState({ name: '', coa_code: 'P-8190', amount: '', cadence: 'monthly', target_date: new Date().toISOString().split('T')[0] });
  const [actionLoading, setActionLoading] = useState<string | null>(null);

  useEffect(() => { loadExpenses(); }, []);
  const loadExpenses = async () => { try { const res = await fetch('/api/personal'); if (res.ok) { const data = await res.json(); setExpenses(data.expenses || []); } } catch (err) { console.error(err); } finally { setLoading(false); } };
  const handleSubmit = async (e: React.FormEvent) => { e.preventDefault(); const res = await fetch('/api/personal', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...form, amount: parseFloat(form.amount) }) }); if (res.ok) { setShowForm(false); setForm({ name: '', coa_code: 'P-8190', amount: '', cadence: 'monthly', target_date: new Date().toISOString().split('T')[0] }); loadExpenses(); } };
  const handleAction = async (id: string, action: 'commit' | 'uncommit') => { if (!confirm(action === 'commit' ? 'Commit?' : 'Uncommit?')) return; setActionLoading(id); await fetch(`/api/personal/${id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action }) }); loadExpenses(); setActionLoading(null); };
  const handleDelete = async (id: string) => { if (!confirm('Delete?')) return; await fetch(`/api/personal/${id}`, { method: 'DELETE' }); loadExpenses(); };
    ...
    <AppLayout>
      <div className="p-6 max-w-6xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <div><h1 className="text-2xl font-bold">👤 Personal</h1><p className="text-gray-600">Lifestyle - dining, entertainment, subscriptions</p></div>
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <Card className="p-4 text-center"><div className="text-sm text-gray-500">Items</div><div className="text-2xl font-bold text-purple-600">{expenses.length}</div></Card>
        </div>
