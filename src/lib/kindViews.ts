...
export const STOPPED_TABLES: ReadonlyArray<{ table: string; why: string }> = [
  ...
  { table: 'finnhub_responses', why: "TRADE-COST-01: the per-symbol Finnhub cache (src/lib/convergence/finnhub-cache.ts) — verbatim vendor answers keyed (symbol, endpoint, params hash) with fetched_at, OVERWRITTEN on refetch, no user, no arrival_id. The book names ONE finnhub feed (fundamentals · REFERENCE, providers.ts) while this table holds seventeen endpoints' answers; which of them the book should name is a rule-book ruling, and the reference view is redefined only by a migration of its own (TABLES-01) — reported here until then" },
];
