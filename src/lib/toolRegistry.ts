...
const FACTS: Readonly<Record<ToolName, ToolFacts>> = {
  ...
  Calendar: {
    ...
    links: [],
    citation: 'discover: src/components/hub/HubCalendar.tsx:192 (/api/calendar) · :208 (/api/operations/daily-plan/items) · :220 (/api/hub/operations-routines) — three GETs, the grid writes nothing · record: src/components/hub/HubCalendar.tsx:199 (the rendered-source filter) · src/lib/calendar/day.ts:304 (dayParts — every row counted into its named part) · src/lib/calendar/sources.ts:85 (manual) · src/components/hub/DayView.tsx:221 (the hand-entered badge) · src/app/api/calendar/events/route.ts:82 (PATCH — a pre-ruling row re-stated and read back) · :123 (DELETE) · no commit: no POST since ONEOFF-01, nothing is authored on the calendar · no decide: no persisted draft',
    note: 'Ten calendar_events sources render, each named with its writer and its reason in src/lib/calendar/sources.ts:83-174; "project" and "routines" are named EXCLUDED at :187-196 because they reach the grid already through their own loaders, and admitting them would draw every block twice. DAY-01 deleted the bare source === "trip" filter this note used to describe. ONEOFF-01 closed the manual writer — its rows are pre-ruling and stay editable; a new one-off is a routine. CAL-01 added the tenth, "reservation": a room that has been PAID for, written when the booking confirms, kept apart from "trip" (what was planned) so the deferred budget retro-map can tell them apart.',
