...
import { WireBytesMissingError, onWireError, onWireRequest, onWireResponse, wireOf } from '../plaid/wire';
...
test('(d) parsed for the SDK, exact bytes on the wire stamp', () => {
  ...
  const res = onWireResponse({ data: body, status: 200, config }, () => t);
...
test('an empty body and an ArrayBuffer body', () => {
  const empty = onWireResponse({ data: Buffer.alloc(0), status: 204 });
  assert.equal(empty.data, null);
  assert.equal(empty.wire?.body.length, 0);
  const bytes = Buffer.from('{"a":1}');
  const ab = bytes.buffer.slice(bytes.byteOffset, bytes.byteOffset + bytes.byteLength);
  const res = onWireResponse({ data: ab, status: 200 });
...
test('a rejected answer keeps its bytes and hands the summarizer parsed JSON; a non-Buffer body is a fault', async () => {
  ...
  const err = Object.assign(new Error('Request failed with status code 429'), { response: { data: errBody as unknown, status: 429, wire: undefined as unknown } });
  ...
  const gateway = Object.assign(new Error('502'), { response: { data: Buffer.from('<html>bad gateway</html>') as unknown, status: 502, wire: undefined as unknown } });
