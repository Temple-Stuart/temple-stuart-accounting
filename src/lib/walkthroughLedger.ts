...
export const WALKTHROUGH_LEDGER: Record<PipePillarId, WalkthroughModule> = {
  ...
  // TRADE-REALITY-2: landing-only order FILTER→SCAN→REVIEW→QUEUE→COMMIT→
  // RECORD (the Landing.tsx LANDING-TRADE-ORDER override — order-parallel
  // with it, not with pipePhases.trade, until the app strip rewires).
  trade: {
    payoff: "Find trades worth taking — and get told when to skip.",
    steps: [
      {
        teach: "Pick what you're hunting",
        you: "Set the rules — universe, gates, strategies",
        app: "Holds your rules — the scan obeys them, always",
        feeds: { target: 2, clause: "your rules" },
        glimpse: "the real scan form, miniaturized — universe chips, direction/premium/risk segments, DTE + Width $, liquidity gates + edge metrics, all 16 strategy chips, Scan",
      },
      {
        teach: "Live market data runs your filters",
        you: "Hit scan",
        app: "Pulls live broker prices + fundamentals + macro + filings and runs YOUR rules",
        feeds: { target: 3, clause: "scored candidates — verdicts attached" },
        glimpse: "the real Pipeline Flow ledger — all 20 steps A–T with their count lines",
      },
      {
      ...
      },
      {
        ...
        you: "Queue your picks — the card holds the prediction",
        app: "Predicted vs Actual, side by side — graded when the fill links",
        feeds: { target: 5, clause: "your fill" },
        glimpse: "the real lab scorecard — PREDICTED vs ACTUAL columns + Link to Position",
      },
      {
        teach: "Every trade lands in your books",
        you: "Pick the legs — one click opens the position",
        app: "Writes the journal entry — the trade becomes a position in your books",
        feeds: { target: 6, clause: "the position" },
        glimpse: "the Trade Commit Workflow from the Books CODE surface — legs, Net, Open Position + what it writes",
      },
      {
        teach: "Wins and losses, written down",
        you: "Nothing — it reads your books",
        app: "Writes every result down — wins AND losses, no cherry-picking",
        feeds: { target: null, clause: "the record" },
        glimpse: "the real track-record heroes — RECORD · NET P&L · MAX-LOSS INTEGRITY + the sync-coverage line, the record line, grade chips and the per-trade detail affordance",
      },
    ],
  },
  tax: {
  ...
  compliance: {
    ...
    steps: [
      ...
      },
      {
        ...
        glimpse: "the audit trail + VERIFY CHAIN → VALID",
      },
    ],
  },
  routines: {
    ...
    steps: [
      ...
      {
        teach: "The record shows it's really happening",
        you: "Just look",
        app: "Keeps every done and every miss from the record — proof, not vibes",
        feeds: { target: null, clause: "the done/missed record" },
        glimpse: "done · due · missed beside each routine, and the next one due",
      },
    ],
  },
  projects: {
