/**
 * DayCalendarView — the PURE, props-only render of the day's blocks as a dense,
 * ONE-LINE stacked list in clock order (NOT an hour-grid).
 *
 * Extracted from DayCalendar (PR B). It owns NO data: no day-feed hook, no
 * fetch, no API call, no data-loading effect, no entity-context hook, no
 * server import. The day feed (`timeline`/`loading`/`error`) and the `entities`
 * list arrive as props — the authed DayCalendar container fetches them and reads
 * the entity context, then passes them in. Pure UI-only state (the collapse
 * `open` toggle + the entity/source filter sets) STAYS here: it triggers no
 * fetch, only re-renders the already-loaded rows. The rendered markup —
 * including the teal scene / indigo task / cyan travel fills — is byte-for-byte
 * equivalent to the pre-extraction DayCalendar output.
 *
 * One shared CSS grid for every row (scene AND task) so columns align vertically.
 * Columns: [ time | name | project | entity | status ]. Source is shown by the
 * fill color. Name wraps (no truncation). Gaps/collisions/unscheduled lane all
 * derive from the passed-in feed.
 */
...
export interface DayCalendarViewProps {
  ...
  // ── Day feed (loaded by the container's day-feed hook) ──────────────────────
  timeline: TimelineRow[];
  loading: boolean;
  error: string | null;
  // ── Entities (read from the container's entity context) ─────────────────────
  entities: Entity[];
}
