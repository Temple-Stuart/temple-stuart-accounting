    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between bg-bg-row border border-border px-4 py-3">
        <div className="text-xs text-text-secondary">
          <span className="font-semibold text-text-primary">{(plan.meals || []).length}</span> meals &middot;{' '}
          <span className="font-semibold text-text-primary">{metrics.totalItems}</span> shopping items &middot;{' '}
      ...
      {DAYS.map((dayName, i) => {
        ...
        return (
          <div key={dayName} className="bg-white border border-border">
            <div className="bg-brand-purple text-white px-4 py-2 text-sm font-semibold flex justify-between">
              <span>{dayName}</span>
              <span className="text-text-faint">{meals.length} meals</span>
            </div>
            ...
            {meals.length > 0 ? (
                  ...
                  <div key={meal.id} className="px-4 py-3">
                    ...
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <div className="flex gap-2">
                        <span className="text-[10px] bg-border px-2 py-1 text-text-secondary">P: {meal.protein}g</span>
                        <span className="text-[10px] bg-border px-2 py-1 text-text-secondary">C: {meal.carbs}g</span>
                        <span className="text-[10px] bg-border px-2 py-1 text-text-secondary">F: {meal.fat}g</span>
                      </div>
                    </div>

                    {meal.description && (
                      <p className="text-xs text-text-secondary mb-2">{meal.description}</p>
                    )}

                    {/* Ingredients */}
                    <div className="mb-2">
                      <div className="text-[10px] text-text-muted uppercase tracking-wider mb-1">Ingredients</div>
                      <ul className="space-y-0.5">
                        {(meal.ingredients || []).map((ing, j) => (
                          <li key={j} className="text-xs text-text-secondary">• {ing.quantity} {ing.unit} {ing.name}</li>
                        ))}
                      </ul>
                    </div>
                    ...
                    {/* Instructions */}
                    <div>
                      <div className="text-[10px] text-text-muted uppercase tracking-wider mb-1">Instructions</div>
                      <ol className="space-y-0.5">
                        {(meal.instructions || []).map((step, j) => (
                          <li key={j} className="text-xs text-text-secondary">
                            <span className="font-medium text-text-primary">{j + 1}.</span> {step}
                          </li>
                        ))}
                      </ol>
                    </div>
              ...
              <div className="px-4 py-6 text-center text-text-faint text-xs">No meals planned</div>
            )}
          </div>
        );
      })}
      ...
      <div className="bg-white border border-border">
        <div className="bg-brand-purple text-white px-4 py-2 flex items-center justify-between">
          ...
          <span className="text-xs text-text-faint">{metrics.itemsWithPrices}/{metrics.totalItems} priced · Est: {fmt(metrics.totalEstimated)}</span>
        </div>
            ...
            {shoppingList.map(item => (
              <tr key={item.id} className="hover:bg-bg-row">
                <td className="px-3 py-2">
                  ...
                  <div className="text-[10px] text-text-faint">{item.notes}</div>
                </td>
                <td className="px-3 py-2 text-text-secondary">{item.packageSize}</td>
                <td className="px-3 py-2 text-text-secondary capitalize">{CATEGORY_CONFIG[item.category]?.label || item.category}</td>
                <td className="px-3 py-2 text-right font-mono text-text-secondary">{fmt(item.estimatedPrice)}</td>
