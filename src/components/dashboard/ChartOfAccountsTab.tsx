  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-sm font-bold">Chart of Accounts</h2>
      ...
      <div className="grid grid-cols-5 gap-4">
        {Object.keys(grouped).map(type => (
          <div key={type} className="bg-white border rounded p-4">
            <div className="text-xs text-text-secondary mb-1">{type.toUpperCase()}</div>
            <div className="text-sm font-bold">{grouped[type].length}</div>
            <div className="text-xs text-text-muted mt-1">accounts</div>
          </div>
        ))}
      </div>
        ...
        <div className="overflow-auto" style={{maxHeight: '600px'}}>
          <table className="w-full">
            <thead className="bg-bg-row sticky top-0">
              <tr>
                <th className="px-4 py-3 text-left text-xs font-semibold text-text-secondary">Code</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-text-secondary">Account Name</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-text-secondary">Type</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-text-secondary">Balance Type</th>
                <th className="px-4 py-3 text-right text-xs font-semibold text-text-secondary">Current Balance</th>
                  ...
                  <tr key={account.id} className="hover:bg-bg-row">
                    ...
                    <td className="px-4 py-3">
                      <span className={`px-2 py-1 rounded text-xs font-medium ${
                        account.accountType === 'asset' ? 'bg-brand-purple-wash text-brand-purple' :
                        account.accountType === 'liability' ? 'bg-red-100 text-brand-red' :
                        account.accountType === 'equity' ? 'bg-purple-100 text-purple-700' :
                        account.accountType === 'revenue' ? 'bg-green-100 text-brand-green' :
                    ...
                    <td className="px-4 py-3 text-right">
                      <span className={`font-semibold ${
                        balance > 0 ? 'text-brand-green' : 
                        balance < 0 ? 'text-brand-red' : 
                        'text-text-faint'
                      }`}>
                    ...
                    <td className="px-4 py-3 text-center">
                      <span className={`px-2 py-1 rounded text-xs font-medium ${
                        account.isArchived ? 'bg-bg-row text-text-secondary' : 'bg-green-100 text-brand-green'
                      }`}>
