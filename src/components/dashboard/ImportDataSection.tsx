...
export function ImportDataSection({ entityId }: { entityId: string }) {
  ...
  const syncCompleteData = async () => {
    setLoading(true);
    setSyncStatus('Syncing ALL transaction data including investment trades...');
                ...
                <div className="flex justify-between items-start">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-xs text-gray-500">
                      Available: ${(account.availableBalance || 0).toLocaleString('en-US', { minimumFractionDigits: 2 })}
                    </p>
                  </div>
                </div>
              </div>
        ...
        <div className="bg-white rounded-lg shadow">
          <div className="border-b">
            <div className="flex">
              ...
              >
                Spending Transactions ({transactions.length})
              </button>
              <button
              ...
              >
                Investment Trades ({investmentTransactions.length})
              </button>
            </div>
          </div>
                ...
                <tbody className="divide-y">
                  {transactions.slice(0, 50).map((txn) => (
                    <tr key={txn.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-2 py-2">
                        {txn.location?.city ? `${txn.location.city}, ${txn.location.region}` : '-'}
                      </td>
                      <td className="px-2 py-2">{txn.payment_channel || '-'}</td>
                      <td className="px-2 py-2">
                        {txn.counterparties?.[0]?.name || '-'}
                      </td>
          ...
          {activeTab === 'investments' && (
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                ...
                <tbody className="divide-y">
                  {investmentTransactions.slice(0, 50).map((txn) => (
                    <tr key={txn.id} className="hover:bg-gray-50">
                      <td className="px-2 py-2">{new Date(txn.date).toLocaleDateString()}</td>
                      <td className="px-2 py-2">{txn.name}</td>
