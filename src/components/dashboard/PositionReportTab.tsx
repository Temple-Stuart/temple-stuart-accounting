...
export default function PositionReportTab({ surface = 'light' }: { surface?: Surface } = {}) {
  ...
  if (loading) {
    return <div className={themed('p-8 text-center text-terminal-sm text-text-muted', dk)}>Loading position report...</div>;
  }
  ...
  return (
    <div>
      {/* Sub-tabs + Refresh */}
      <div className={themed('flex items-center border-b border-border', dk)}>
        {[
        ...
        <button onClick={loadData} className={themed('ml-auto mr-2 px-3 py-1 text-xs border border-border text-text-secondary rounded-lg hover:bg-bg-row transition-colors', dk)}>
          Refresh
        </button>
      </div>
        ...
        <div className="p-3 space-y-3">
          ...
          <div className="grid grid-cols-5 gap-2">
            <div className={themed('border border-border rounded p-2', dk)}>
              <div className={themed('text-terminal-xs text-text-muted uppercase tracking-widest', dk)}>Total Realized P&L</div>
              <div className={`text-terminal-lg font-bold font-mono ${plColor(summary.totalRealizedPL)}`}>
                {fmt(summary.totalRealizedPL)}
              </div>
            </div>
            <div className={themed('border border-border rounded p-2', dk)}>
              <div className={themed('text-terminal-xs text-text-muted uppercase tracking-widest', dk)}>Short-Term P&L</div>
              <div className={`text-terminal-lg font-bold font-mono ${plColor(summary.shortTermPL)}`}>
                {fmt(summary.shortTermPL)}
              </div>
            </div>
            <div className={themed('border border-border rounded p-2', dk)}>
              <div className={themed('text-terminal-xs text-text-muted uppercase tracking-widest', dk)}>Long-Term P&L</div>
              <div className={`text-terminal-lg font-bold font-mono ${plColor(summary.longTermPL)}`}>
                {fmt(summary.longTermPL)}
              </div>
            </div>
            <div className={themed('border border-border rounded p-2', dk)}>
              <div className={themed('text-terminal-xs text-text-muted uppercase tracking-widest', dk)}>Win Rate</div>
              <div className="text-terminal-lg font-bold font-mono">{summary.winRate}%</div>
            </div>
            <div className={themed('border border-border rounded p-2', dk)}>
              <div className={themed('text-terminal-xs text-text-muted uppercase tracking-widest', dk)}>Profit Factor</div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className={themed('border border-border rounded p-3', dk)}>
            ...
            <div className={themed('border border-border rounded p-3', dk)}>
              ...
              <div className={themed('flex gap-4 mt-2 text-terminal-sm text-text-muted', dk)}>
          ...
          {data.byStrategy.length > 0 && (
            <div className={themed('border border-border rounded overflow-hidden', dk)}>
              <div className={themed('bg-bg-row px-3 py-1.5 text-terminal-lg font-semibold', dk)}>By Strategy</div>
              <table className="w-full text-terminal-base">
                ...
                <tbody>
                  {data.byStrategy.map((s, idx) => (
                    <tr key={s.strategy} className={themed(`border-b border-border-light hover:bg-bg-row ${idx % 2 === 0 ? 'bg-white' : 'bg-bg-row'}`, dk)}>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className={themed('border border-border rounded p-2', dk)}>
              <div className={themed('text-terminal-xs text-text-muted uppercase tracking-widest', dk)}>Avg Win</div>
              <div className="text-base font-bold font-mono text-brand-green">{fmt(summary.avgWin)}</div>
            </div>
            <div className={themed('border border-border rounded p-2', dk)}>
              <div className={themed('text-terminal-xs text-text-muted uppercase tracking-widest', dk)}>Avg Loss</div>
      ...
      {activeView === 'open' && (
        <div className="p-3 space-y-3">
          ...
          {data.openPositions.options.length > 0 && (
            <div className={themed('border border-border rounded overflow-hidden', dk)}>
              <div className={themed('bg-bg-row px-3 py-1.5 text-terminal-lg font-semibold text-text-primary', dk)}>
                ...
                <table className="w-full text-terminal-base">
                  ...
                  <tbody>
                    {data.openPositions.options.map((p, idx) => (
                      <tr key={p.id} className={themed(`border-b border-border-light hover:bg-bg-row ${idx % 2 === 0 ? 'bg-white' : 'bg-bg-row'}`, dk)}>
                        ...
                        <td className={themed('py-1 px-2 font-mono text-text-muted', dk)}>{fmtDate(p.expiration)}</td>
                        ...
                        <td className={themed('py-1 px-2 font-mono text-text-muted', dk)}>{fmtDate(p.openDate)}</td>
                        <td className="py-1 px-2">{p.strategy}</td>
                        <td className={themed('py-1 px-2 text-right font-mono text-text-faint', dk)}>#{p.tradeNum}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                ...
                <table className="w-full text-terminal-base">
                  ...
                  <tbody>
                    {data.openPositions.stocks.map((p, idx) => (
                      <tr key={p.symbol} className={themed(`border-b border-border-light hover:bg-bg-row ${idx % 2 === 0 ? 'bg-white' : 'bg-bg-row'}`, dk)}>
                        <td className="py-1 px-2 font-medium">{p.symbol}</td>
                        ...
                        <td className={themed('py-1 px-2 font-mono text-text-muted', dk)}>{fmtDate(p.openDate)}</td>
                        <td className={themed('py-1 px-2 text-right font-mono text-text-faint', dk)}>{p.lotCount}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          {data.openPositions.options.length === 0 && data.openPositions.stocks.length === 0 && (
            <div className={themed('text-center text-terminal-sm text-text-muted py-8', dk)}>No open positions</div>
          )}
        </div>
      )}
      ...
      {activeView === 'closed' && (
        <div className="p-3 space-y-3">
                  ...
                  <tbody>
                    {data.closedPositions.options.map((p, idx) => (
                      <tr key={p.id} className={themed(`border-b border-border-light hover:bg-bg-row ${idx % 2 === 0 ? 'bg-white' : 'bg-bg-row'}`, dk)}>
                        ...
                        <td className={themed('py-1 px-2 text-right font-mono text-text-muted', dk)}>{p.holdingDays ?? '-'}</td>
                        <td className="py-1 px-2">
                          <span className={`px-1.5 py-0.5 rounded text-[10px] font-medium ${
                        ...
                        <td className={themed('py-1 px-2 font-mono text-text-muted', dk)}>{fmtDate(p.closeDate)}</td>
                        <td className={themed('py-1 px-2 text-right font-mono text-text-faint', dk)}>#{p.tradeNum}</td>
                      </tr>
                    ))}
                  </tbody>
          ...
          {data.closedPositions.options.length === 0 && data.closedPositions.stocks.length === 0 && (
            <div className={themed('text-center text-terminal-sm text-text-muted py-8', dk)}>No closed positions</div>
          )}
        </div>
      )}
    </div>
