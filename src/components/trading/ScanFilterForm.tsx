    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-8 gap-y-4 border-t border-border pt-3">
        ...
        <div>
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2"><FieldLabel>{MIN_POP_MODEL_LABEL}</FieldLabel><input type="range" min={0} max={100} value={f.edge.minPop} onChange={e => onFiltersChange({ ...f, edge: { ...f.edge, minPop: +e.target.value } })} className="flex-1 h-1.5 accent-brand-purple cursor-pointer" /><span className="text-[11px] font-mono font-semibold text-text-primary w-[54px] text-right">{f.edge.minPop}%</span></div>
            <div className="flex items-center gap-2"><FieldLabel>{MIN_EV_MODEL_LABEL}</FieldLabel><input type="range" min={-500} max={1000} step={10} value={f.edge.minEv} onChange={e => onFiltersChange({ ...f, edge: { ...f.edge, minEv: +e.target.value } })} className="flex-1 h-1.5 accent-brand-purple cursor-pointer" /><span className="text-[11px] font-mono font-semibold text-text-primary w-[54px] text-right">${f.edge.minEv}</span></div>
            <div className="flex items-center gap-2"><FieldLabel>{MIN_EV_PER_RISK_MODEL_LABEL}</FieldLabel><input type="range" min={-200} max={200} step={5} value={Math.round(f.edge.minEvPerRisk * 100)} onChange={e => onFiltersChange({ ...f, edge: { ...f.edge, minEvPerRisk: +e.target.value / 100 } })} className="flex-1 h-1.5 accent-brand-purple cursor-pointer" /><span className="text-[11px] font-mono font-semibold text-text-primary w-[54px] text-right">{f.edge.minEvPerRisk.toFixed(2)}</span></div>
