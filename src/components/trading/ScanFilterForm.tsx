...
export default function ScanFilterForm({
      ...
      <div className={themed('grid grid-cols-1 lg:grid-cols-2 gap-x-8 gap-y-4 border-t border-gray-100 pt-3', dk)}>
        ...
        <div>
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-2"><FieldLabel>Vol Edge</FieldLabel><div className={themed('flex gap-px rounded overflow-hidden border border-gray-200', dk)}>{(['IV_ABOVE_HV', 'IV_BELOW_HV', 'ANY'] as const).map(v => (<button key={v} type="button" onClick={() => onFiltersChange({ ...f, edge: { ...f.edge, volEdge: v } })} className={themed(`px-2 py-0.5 text-[10px] font-bold ${f.edge.volEdge === v ? 'bg-brand-purple text-white' : 'bg-white text-gray-400 hover:bg-gray-50'}`, dk)}>{v === 'IV_ABOVE_HV' ? 'IV>HV' : v === 'IV_BELOW_HV' ? 'IV<HV' : 'Any'}</button>))}</div></div>
          ...
          {AVAILABLE_STRATEGIES.map(s => {
            ...
            return (
              <button key={s} type="button" onClick={() => {
                ...
                className={themed(`px-2 py-0.5 rounded text-[11px] font-medium ${active ? 'bg-brand-purple text-white' : 'bg-gray-100 text-gray-400 border border-gray-200'}`, dk)}>
                {s}
              </button>
            );
          })}
  ...
  return (
    <div className={themed('rounded-lg overflow-hidden border border-gray-200/50 shadow-sm mb-3', dk)}>
      ...
      <div className={themed('bg-white p-4', dk)}>{formBody}</div>
    </div>
  );
}
